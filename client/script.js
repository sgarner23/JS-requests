...
sayHelloButton.addEventListener("mouseover", changeBackgroudColor);
...
const repeatMyParam = () => {
  ...
  axios.get(`http://localhost:3000/repeat/steak`).then((res) => {
...
function queryRequest(event) {
  axios
    .get("http://localhost:3000/query-test?name1=stephen&name2=bruno")
    .then((res) => {
      console.log(res.data);
    });
}

const idBtn = document.querySelector("#query-button");

idBtn.addEventListener("click", queryRequest);
